...
def test_display_jobs_headers():
    ...
    assert display_jobs([]) == (
        ...
        'ID очереди | Название очереди | Название аллокации |\n+------------+--------+---------+---------------+-----------------+------'
        '----------+--------------+------------+------------------+--------------------+\n+------------+--------+---------+--------------'
        '-+-----------------+----------------+--------------+------------+------------------+--------------------+'
    )
...
def test_display_jobs_body(sample):
    ...
    assert display_jobs(sample) == (
        ...
        '--------------+------------------+--------------------------------------+------------------+--------------------+\n|       '
        '            Имя '
        'задачи                    | Статус  |  Регион | Instance Type |      Описание задачи       | Количество GPU |   Длительность   |'
        '              ID очереди              | Название очереди | Название аллокации |\n+---------------------------------------------'
        '----+---------+---------+---------------+----------------------------+----------------+------------------+---------------------'
        '-----------------+------------------+--------------------+\n| lm-mpi-job-d305a5a8-12c6-4a65-801d-000000000000 | Pending | A100-MT'
        ' |   v100.1gpu   | set any useful description |       1        | 8 days, 19:31:01 | 00000000-0000-0000-0000-000000000000 |  '
        ' hello world    |                    |\n+-------------------------------------------------+---------+---------+---------------'
        '+----------------------------+----------------+------------------+--------------------------------------+------------------+--'
        '------------------+'
    )
