...
def test_table_command_basic(runner, mock_api_job, monkeypatch):
    ...
    with runner.isolated_filesystem():
        ...
        assert result.output == (
            '+------------+---------+---------+---------------+------------------+----------------+--------------+'
            '------------+------------------+--------------------+\n| Имя задачи | Статус  |  Регион | Instance Type'
            ' | Описание задачи  | Количество GPU | Длительность | ID очереди | Название очереди | Название аллокации'
            ' |\n+------------+---------+---------+---------------+------------------+----------------+--------------+'
            '------------+------------------+--------------------+\n|  test-job  | running |         |   gpu-large   | '
            'test description |       2        |   0:00:00    |            |                  |                    |\n+-'
            '-----------+---------+---------+---------------+------------------+----------------+--------------+----------'
            '--+------------------+--------------------+\n'
        )
...
def test_table_command_basic_filter(runner, mock_api_job, monkeypatch):
    ...
    with runner.isolated_filesystem():
        ...
        assert result.output == (
            '+------------+--------+---------+---------------+-----------------+----------------+--------------+------------+'
            '------------------+--------------------+\n| Имя задачи | Статус |  Регион | Instance Type | Описание задачи | '
            'Количество GPU | Длительность | ID очереди | Название очереди | Название аллокации |\n+------------+--------+--'
            '-------+---------------+-----------------+----------------+--------------+------------+------------------+------'
            '--------------+\n+------------+--------+---------+---------------+-----------------+----------------+------------'
            '--+------------+------------------+--------------------+\n'
